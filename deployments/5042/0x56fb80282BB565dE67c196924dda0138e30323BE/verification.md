...
Registry status: `deployed-pending-activation`. This is an operational label, not an on-chain gate: the vault is **unpaused**. The owner and settlement authority have empty code, nonce zero and zero native USDC at the snapshot. No funded mainnet deposit/redeem smoke test has been performed. Arc Etherscan reports exact-match source verification for all five application contracts. This report does not authorize deposits or replace an independent security audit.

On-chain verification used only read-only RPC and `eth_call`; no blockchain transaction was signed or broadcast by the verifier. Source and constructor metadata were separately published to Arc Etherscan without a wallet or gas. `eth_call` with an explicit sender proves the contract's permission checks, not control of that sender's key or its ability to submit transactions.
...
- Underlying asset: user-specified [`0x171A4217b86A807A64eB94757Db6849fb4bDbAA0`](https://arc.etherscan.io/token/0x171A4217b86A807A64eB94757Db6849fb4bDbAA0); on-chain metadata reports `Circle Wrapped Bitcoin` / `cirBTC` and 8 decimals. This is an exact address/metadata check, not independent attestation of the issuer.
...
| Component | Address |
| --- | --- |
| AtomicDeployment (coordinator) | [`0x90E1cac2D985E9A961f043b9682EcCe72E86f8F1`](https://arc.etherscan.io/address/0x90E1cac2D985E9A961f043b9682EcCe72E86f8F1#code) |
| SmartAccountWrapper (implementation) | [`0xBF67E0CC43285FF4392bC37b6717f522E4c8e824`](https://arc.etherscan.io/address/0xBF67E0CC43285FF4392bC37b6717f522E4c8e824#code) |
| UpgradeableBeacon (beacon) | [`0xc77Fb640FD7D6B056Fa30C74cB9eF65B3636C4fA`](https://arc.etherscan.io/address/0xc77Fb640FD7D6B056Fa30C74cB9eF65B3636C4fA#code) |
| BeaconProxy (wrapperProxy) | [`0x56fb80282BB565dE67c196924dda0138e30323BE`](https://arc.etherscan.io/address/0x56fb80282BB565dE67c196924dda0138e30323BE#code) |
| Staging (staging) | [`0x778ef6d801672009bCd878f69055f56907fa2217`](https://arc.etherscan.io/address/0x778ef6d801672009bCd878f69055f56907fa2217#code) |

## Successful atomic deployment

[`0x3eddb609c2efa695fd003bf8e1ad0f2146c1b3cc2a43e9ecfc336a1bf47aae49`](https://arc.etherscan.io/tx/0x3eddb609c2efa695fd003bf8e1ad0f2146c1b3cc2a43e9ecfc336a1bf47aae49)
...
A read-back through Etherscan API V2 and each public `#code` page at `2026-09-24T10:55:39Z` reported exact-match source verification for SmartAccountWrapper, UpgradeableBeacon, BeaconProxy, Staging and AtomicDeployment. Each record reports compiler `v0.8.30+commit.73712a01`, EVM `osaka`, optimizer enabled with 200 runs, the expected contract name, and constructor arguments matching the immutable deployment inputs. See [`etherscan-status.json`](etherscan-status.json), SHA-256 `559a58adb05a5c62c4e56488e1b5c1b6d0cbb0fa3462a75e8369a6d33c069800`.

The earlier Arc Blockscout check remains a separate historical snapshot: SmartAccountWrapper, BeaconProxy and Staging were fully verified there, UpgradeableBeacon was partially verified, and AtomicDeployment was not verified. See [`explorer-status.json`](explorer-status.json), SHA-256 `a5477768c871d76c9910ac12cf2f42cdbd24b2def3645e9c0df8a69fed349c75`. Explorer publication is distinct from the exact local bytecode provenance checks below; Etherscan verification does not change Blockscout's independent status.
...
4. Preserve the completed Arc Etherscan publication and read-back evidence for all five application contracts. Arc Blockscout retains its earlier partial/incomplete status, but that separate explorer status is not a deployment mismatch or a reason to redeploy.
